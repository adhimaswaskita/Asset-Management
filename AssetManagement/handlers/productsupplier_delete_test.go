...
func TestDeleteProductSupplier(t *testing.T) {
	...
	r := mux.NewRouter()
	r.HandleFunc("/productsupplier", handler.DeleteProductSupplier).Methods("DELETE")
	...
	t.Run("Delete product supplier OK", func(t *testing.T) {
		...
		req, _ := http.NewRequest("DELETE", server.URL+"/productsupplier?ID="+stringID, nil)
	...
	t.Run("Create Product supplier NOK", func(t *testing.T) {
		service.ErrMap["ErrorDeleteProductSupplier"] = true
		req, _ := http.NewRequest("DELETE", server.URL+"/productsupplier", nil)
		resp, err := http.DefaultClient.Do(req)
		if err != nil {
